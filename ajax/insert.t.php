...
{  
    ...
    isset($_POST["ps"]) ? $_POST["ps"] !== '' ? $ps = $_POST["ps"] : $ps = null : $ps = null; // producing status -- status of well when note was made
    isset($_POST["ft"]) ? $_POST["ft"] !== '' ? $ft = $_POST["ft"] : $ft = null : $ft = null; // file type -- ie. is it for a lease road, well, generic document, etc
   ...
   if($d == 'a' || $d == 'v') {
    ...
    if($na == 'append'){
        ...
        $notes_stmt = $connect->prepare("UPDATE list SET producing_status=?, notes=?, ts=?, de=? WHERE api = ?");
        $notes_stmt->bind_param('sssss', $ps, $newNotes, $ts, $de, $api);
        $notes_stmt->execute(); 
    }
    elseif($na == 'replace'){
        $notes_stmt = $connect->prepare("UPDATE list SET producing_status=?, notes=?, ts=?, de=? WHERE api = ?");
        $notes_stmt->bind_param('sssss', $ps, $drn, $ts, $de, $api);
        $notes_stmt->execute(); 
    }
    $vitals = 0;
    ...
    {
        $stmt = $connect->prepare("INSERT INTO notes (d, t, de, ts, te, deb, api, cvn, cin, drn, ai, ad, edc, ecc, tt, dt, dc, at, ac, et, vb, producing_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param('ssssssssssssddddddddis', $d, $t, $de, $ts, $te, $deb, $api, $cvn, $cin, $drn, $ai, $ad, $edc, $ecc, $tt, $dt, $dc, $at, $ac, $et, $vitals, $ps);
    ...
    isset($_POST["wsbcategory"]) ? $_POST["wsbcategory"] !== '' ? $wsbcat = $_POST["wsbcategory"] : $wsbcat = "Not Categorized" : $wsbcat = "Not Categorized";
    $wsbslug = Slug($wsbcat);
    print_r($wsbslug);
    $checkcat = "SELECT * FROM `wsbcategories` WHERE slug='$wsbslug'";
    $getWSBCat = mysqli_query($connect, $checkcat);
    print_r($getWSBCat);
    if(mysqli_num_rows($getWSBCat) == 0)
    { 
        $stmt = $connect->prepare("INSERT INTO wsbcategories (category, slug) VALUES (?, ?)");
        $stmt->bind_param('ss', $wsbcat, $wsbslug);

        $stmt->execute();
        console_log("New category inserted successfully.");
    }
    ...
    if(!empty($fileNames)){ 
            ...
            if(in_array($fileType, $allowTypes)){ 
               // Upload file to server 
                if(move_uploaded_file($_FILES["files"]["tmp_name"][$key], $fullFilePath)){ 
                   // Image db insert sql 
                    $insertValuesSQL .= "('".$api."', '".$id."', '".$fileName."', '".mysqli_real_escape_string($connect,$targetFilePath)."', NOW(), '".$ft."'),"; 
                }else{ 
        ...
        if(!empty($insertValuesSQL)){ 
           ...
           // Insert image file name into database
            $sql = "INSERT INTO notes_files (`api`, `note_id`, `filename`, `filepath`, `sd`, `t`) VALUES $insertValuesSQL";
            console_log($sql); 
    ...
    //*/
      ...
      // This checks for the current wsb files subdirectory categories, and adds a new category if there isn't one
      
    
}  
else
{
    print_r("returned empty");
}
?>
