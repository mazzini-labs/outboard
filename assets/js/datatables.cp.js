$(function() {
    ...
    var drn = {
        selector: '#drn1',
        skin: "CUSTOM",
        // content_css: "https://vprsrv.org/js/tinymce/skins/content/CUSTOM",
        menubar: false,
        inline: true,
        plugins: [
            'lists',
        ...
        // numlist bullist 
        contextmenu: 'copy paste | undo redo | help',
    ...
    // .click(function(){
        if(clickt1 < 1){
            clickt1++;
            aDDRTable = $('#cTable').on( 'init.dt', function () {
                aDDRTable.scroller().scrollToRow(100000);
            } )
            .DataTable( {
                "ajax": 
                {
                    "url" : "ajax/cp.ddr.fetch.php",
                    // "type" : "POST",
                    "data": 
                    {
                        "api": window.api,
    ...
    $('#d-tab').on('shown.bs.tab', function (e) {
        if(clickd1 < 1){
            clickd1++;
            dDDRTable = $('#dTable').on( 'init.dt', function () {
                dDDRTable.scroller().scrollToRow(100000);
            } )
            .DataTable( {
                "ajax": 
                {
    ...
    $('#s-tab').on('shown.bs.tab', function (e) {
        if(clicks1 < 1){
            clicks1++;
            sDDRTable = $('#sTable').on( 'init.dt', function () {
                sDDRTable.scroller().scrollToRow(100000);
            } )
            .DataTable( {
                "ajax": 
                {
    ...
    // .click(function(){
        if(clickt3 < 1){
            clickt3++;
            bDDRTable = $('#compTable').on( 'init.dt', function () {
    ...
    $('#smh-tab').on('shown.bs.tab', function (e) {
        if(clicksmh1 < 1){
            clicksmh1++;
            smhDDRTable = $('#smhTable').on( 'init.dt', function () {
                smhDDRTable.scroller().scrollToRow(100000);
            } )
            .DataTable( {
                "ajax": {
                "url" : "ajax/cp.ddr.fetchold.php",
    ...
    $('#dehy-tab').on('shown.bs.tab', function (e) {
        if(clickdehy1 < 1){
            clickdehy1++;
            dehyDDRTable = $('#dehyTable').on( 'init.dt', function () {
                dehyDDRTable.scroller().scrollToRow(100000);
            } )
            .DataTable( {
                "ajax": {
                "url" : "ajax/cp.ddr.fetchold.php",
    ...
    $('#txdot-tab').on('shown.bs.tab', function (e) {
        if(clicktxdot1 < 1){
            clicktxdot1++;
            txdotDDRTable = $('#txdotTable').on( 'init.dt', function () {
                txdotDDRTable.scroller().scrollToRow(100000);
            } )
            .DataTable( {
                "ajax": {
                "url" : "ajax/cp.ddr.fetchold.php",
