...
echo "<style>"
echo "#outboard{ \n";
echo "  font-family: 'Roboto', 'Trebuchet MS', Arial, Helvetica, sans-serif; \n";
echo "  font-size: 14px; \n";
echo "  text-rendering: optimizeLegibility; \n";
echo "  border-collapse: collapse; \n";
echo "  border-left: 1px solid 3e94ec; \n";
echo "  border-radius: 30px; \n";
echo "  width: 100%; \n";
echo "  font-weight: normal; \n";
echo "  text-shadow: 0 1px 1px rgba(256, 256, 256, 0.1); \n";
echo "  box-shadow:  0 6px 20px 0 rgba(0, 0, 0, 0.19); \n";
echo "  text-align: left; \n";
echo "} \n";
echo "#outboard td, #outboard th { \n";
echo "  padding: 1px; \n";
echo "  border-style: solid; \n";
echo "  border-width: 0px 2px 0px 2px; \n";
echo "  border-color: #1b1e24; \n";
echo "} \n";
...
echo "<div class='container-outboard100'>\n";
echo "<div class='wrap-outboard100'>\n";
echo "<table id=outboard width=100% border=0 cellpadding=7 cellspacing=1>\n";
...
echo "      <table id=outboard width=200 border=0 cellpadding=5 cellspacing=0>\n";
...
while ($row=mysql_fetch_array($result)) {
  $tmp_empfullname = stripslashes("".$row['userid']."");
...
if ($fullname != "All") {
$query = "select userid, name from ".$db_prefix."outboard where name = '".$fullname."'";
$result = mysql_query($query);

while ($row=mysql_fetch_array($result)) {
$empfullname = stripslashes("".$row['userid']."");
$displayname = stripslashes("".$row['name']."");
}
...
if (strtolower($user_or_display) == "display") {
    ...
    if (($office_name == "All") && ($group_name == "All") && ($fullname == "All")) {
        ...
        $query = "select userid, name from ".$db_prefix."outboard WHERE name <> 'admin'
                  order by name asc";
        $result = mysql_query($query);
        ...
        $query = "select userid, name from ".$db_prefix."outboard WHERE name = '".$fullname."' 
                  and name <> 'admin' order by name asc";
        $result = mysql_query($query);

    } /*elseif (($office_name != "All") && ($group_name == "All") && ($fullname == "All")) {

        $query = "select userid, name from ".$db_prefix."outboard where office = '".$office_name."' and tstamp IS NOT NULL 
                  and name <> 'admin' order by displayname asc";
        $result = mysql_query($query);
	...
	*/
    ...
    if (($office_name == "All") && ($group_name == "All") && ($fullname == "All")) {
        ...
        $query = "select userid, name from ".$db_prefix."outboard WHERE tstamp IS NOT NULL and name <> 'admin' 
                  order by name asc";
        $result = mysql_query($query);
        ...
        $query = "select userid, name from ".$db_prefix."outboard where office = '".$office_name."' and groups = '".$group_name."' 
