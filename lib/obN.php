<?php
// require_once("lib/OutboardDatabaseN.php");
require_once("lib/obDB.php");
...
$ob   = new OutboardDatabase($localhost);
...
print_r(" Database: " . $ob->getConfig('db'));
...
if ($ob->getConfig('authtype') == "internal") {
  // print_r(getPostValue('username'));
  // print_r(getPostValue('password'));
  ...
  // else {
  //   print_r("one of username (".$username.") or password (".$password.") is not set.");
  // }
} else {
  print_r("basic");
  $BasicAuthInUse = true;
