...
<SCRIPT Language="JavaScript1.2">
  function deleteConfirm(user,name,unique) {
    if (confirm("Delete extension for user "+user+"?")) {
      mylocation =
	"<?php echo $baseurl ?>?adminscreen=1&deletethisext="
	  + name 
	  +"&unique=<?php echo $unique ?>" + "&table=phoneextensions";
...
if ($ob->getPEData()) {
  while ($row = $ob->getRow()) {
    ...
    echo "<div class='col-1'>".$row['extension']."</div>";
