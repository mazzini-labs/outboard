...
$editor = Editor::inst( $db, 'list', 'list_id' )
	->fields(
		...
		Field::inst( 'first_prod_date' )
			->validator( Validate::dateFormat( 'Y-m-d' ) )
			->getFormatter( Format::dateSqlToFormat( 'Y-m-d' ) )
            ->setFormatter( Format::dateFormatToSql('Y-m-d' ) ),
		Field::inst( 'drill_type')
			->validator( Validate::values( array('V', 'H', 'U') ) ),	
		Field::inst( 'upper_perforation'),
		Field::inst( 'lower_perforation'),
		Field::inst( 'gas_gravity' ),
		Field::inst( 'completion_date' ),
		Field::inst( 'well_count' ),
		Field::inst( 'max_active_wells' ),
		Field::inst( 'months_produced' ),
		Field::inst( 'gas_gatherer' ),
		Field::inst( 'oil_gatherer' ),
		Field::inst( 'spud_date' ),
		Field::inst( 'measured_depth_td' ),
		Field::inst( 'true_vertical_depth' ),
		Field::inst( 'field' ),
		Field::inst( 'reservoir'),
		Field::inst( 'surface_latitude_wgs84' ),
		Field::inst( 'surface_longitude_wgs84' ),
		Field::inst( 'report_frequency'),
		Field::inst( 'well_file_location'),
		Field::inst( 'show'),
