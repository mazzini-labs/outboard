...
if ($submit) {   // Run this if the user pressed the Install button
  ...
  $selectDB = @mysqli_select_db($dbh, $db);
  if (! $dbh || ! $selectDB) { 
    ...
    $result = @mysqli_query($dbh,$outboard_table);
    if ($result) {
      echo "Done.<p>";
      echo "---------------------------------------------------<br>";
      echo "Creating <i>$logtable</i> table... ";
      $result = @mysqli_query($dbh,$outboard_log_table);
      if ($result) {
        echo "Done.<p>";
        echo "---------------------------------------------------<br>";
        ...
        $result = mysqli_query($dbh,"INSERT into $table (userid,password,name,options) "
        		."VALUES ('$adminuser',CONCAT('*', UPPER(SHA1(UNHEX(SHA1('$adminpass'))))),'$adminname',' ADMIN ')");
        echo "Done.<p>";
