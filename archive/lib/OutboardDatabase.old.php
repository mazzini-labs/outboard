...
Function OutboardDatabase() {
  ...
  // $this->dbs = "mysql:host=".$this->getConfig('dbhost').";dbname=".$this->getConfig('db');
  // $this->dbs .= ";charset=utf8";
  // $this->db = new PDO($this->dbs, $this->getConfig('dbuser'), $this->getConfig('dbpass'));
...
Function deleteExt($field,$value,$table) {
...
Function _query($stmt) {
  ...
  // if ($this->result = $this->db->query($stmt)) {
    return true;
  } else {
...
Function numRows() {
  ...
  // return $this->db->count($this->result);
}

Function getRow() {
  if (! $this->result) { return null; }
  if ($row = mysql_fetch_array($this->result)) { 
  // if ($row = $this->db->fetch($this->result)) { 
    return $row;
  } else {
    return null;
  } 
