<?php
require_once("ajax/obcontroller.php");
$db_handle = new DBController();

$sql = "SELECT * from new_ddr";
...
function saveToDatabase(editableObj,column,id) {
  $(editableObj).css("background","#FFF url(./ajax/loaderIcon.gif) no-repeat right");
  $.ajax({
    url: "./ajax/edit.php",
    type: "POST",
...
function addToDatabase() {
  ...
  var description = $("#description").val();
	  ...
	  $("#confirmAdd").html('<img src="./ajax/loaderIcon.gif" />');
	  $.ajax({
		url: "./ajax/add.php",
		type: "POST",
  ...
  <tbody id="table-body">
	...
	foreach($posts as $k=>$v) {
	  ...
	  <tr class="table-row" id="table-row-<?php echo $posts[$k]["id"]; ?>">
		<td contenteditable="true" onBlur="saveToDatabase(this,'post_title','<?php echo $posts[$k]["id"]; ?>')" onClick="editRow(this);"><?php echo $posts[$k]["department"]; ?></td>
		<td contenteditable="true" onBlur="saveToDatabase(this,'description','<?php echo $posts[$k]["id"]; ?>')" onClick="editRow(this);"><?php echo $posts[$k]["type"]; ?></td>
