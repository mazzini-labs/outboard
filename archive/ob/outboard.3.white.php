        ...
        <thead><tr>
          ...
          while($row = $ob->getRow()) {
            ...
            if ($row['userid'] == $username && $update && $isChangeable) {
              ...
              if ($update && $isChangeable) {
                echo "<td $isChangeable $user_bg'><a style='color: black;' href=\"javascript:this.change_remark('"
                . addslashes(htmlspecialchars($row['remarks']))
                . addslashes($row['remarks'])
                . "','".$row['userid']."')\">$print_remarks</a></td>";
                // console_log($isChangeable);
              } else {
...
<?php if (! $update && ! $readonly) { ?>
          <a class="btn-floating btn-lg btn-info bg-danger cs" href="/outboard.3.white.php?update=1#<?php echo $username ?>"> <!-- data-toggle="tooltip" data-placement="left" title="Change Status"> -->
          <i class="lg-icon" data-feather="edit-3" ></i>
        <?php } elseif (! $readonly) { ?>
          <a class="btn-floating btn-lg btn-info bg-sog vs" href="/outboard.3.white.php?noupdate=1"> <!-- data-toggle="tooltip" data-placement="left" title="View Status"> -->
