...
Mint.save = function() 
{
	...
	var path	= 'http://calculator.me/mint/?record&key=7678473952424e6c3438373171653034477752776874383531';
...
{
	return /^([^:]+):\/\/([a-z0-9]+[\._-])*(calculator\.me)/i.test(url);
};
function Mint_SI_IO_save()
{
	var now		= new Date();
	var then	= now.getTime() + 300;
	var path	= 'http://calculator.me/mint/pepper/shauninman/outbound/click.php?'+now.getTime();
	path 		= path.replace(/^https?:/, window.location.protocol);
