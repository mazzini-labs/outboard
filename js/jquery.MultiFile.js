				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					$.each(files, function (i, file) {
						var v = String(file.name || '' ).replace(/[&<>'"]/g, function(c) { return '&#'+c.charCodeAt()+';'; }),
								...
								p = file.type.substr(0,6) == 'image/' ? '<img class="MultiFile-preview col" style="'+ MultiFile.previewCss+'"/>' : ''
						var btn;
						if(file.type.substr(0,6) == 'image/'){
							p = '<img class="MultiFile-preview col" style="'+ MultiFile.previewCss+'"/>';
							btn = 'btn-secondary';
						} else if (getFileExtension(file.name) == 'doc' || getFileExtension(file.name) == 'docx') {
							p = '<i class="MultiFile-preview col fas fa-file-word fa-3x py-2"></i>';
							btn = 'btn-word';
						} else if (getFileExtension(file.name) == 'xls' || getFileExtension(file.name) == 'xlsx') {
							p = '<i class="MultiFile-preview col fas fa-file-excel fa-3x py-2" style="'+ MultiFile.previewCss+'"></i>';
							btn = 'btn-excel';
						} else if (getFileExtension(file.name) == 'ppt' || getFileExtension(file.name) == 'pptx') {
							p = '<i class="MultiFile-preview col fas fa-file-powerpoint fa-3x py-2" style="'+ MultiFile.previewCss+'"></i>';
							btn = 'btn-powerpoint';
						} else if (getFileExtension(file.name) == 'pdf') {
							p = '<i class="MultiFile-preview col fas fa-file-pdf fa-3x py-2" style="'+ MultiFile.previewCss+'"></i>';
							btn = 'btn-pdf';
						} else {
							p = '<i class="MultiFile-preview col fas fa-file-alt fa-3x py-2" style="'+ MultiFile.previewCss+'"></i>';
							btn = 'btn-secondary';
						} 
						
						var label =	$(
							(
								'<span class="MultiFile-label row btn ' + btn +'" title="' + t + '">'+
									'<span class="MultiFile--title col"><p class="MultiFile-subtitle">'+ n +'</p></span>'+
						...
						label.find('.MultiFile-preview').each(function(){
